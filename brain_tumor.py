...
def load_and_preprocess_images(folder_path, image_size=(128, 128)):
    ...
    class_names = ['glioma', 'meningioma', 'notumor',
                   'pituitary']  # Folder names as class labels
...
def build_generator():
    model = keras.Sequential([
        ...
        # Upsample to 64x64
        layers.Conv2DTranspose(32, (5, 5), strides=(
            2, 2), padding='same', use_bias=False),
        layers.BatchNormalization(),
        layers.LeakyReLU(),
...
def train_step(images):
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        ...
        disc_loss = cross_entropy(tf.ones_like(
            real_output), real_output) + cross_entropy(tf.zeros_like(fake_output), fake_output)
...
def train(dataset, epochs):
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(
                f"Epoch {epoch+1}/{epochs}, Gen Loss: {gen_loss:.4f}, Disc Loss: {disc_loss:.4f}")


# Prepare the dataset
buffer_size = max(1000, x_train.shape[0])  # Avoid zero buffer size
train_dataset = tf.data.Dataset.from_tensor_slices(
    x_train).shuffle(buffer_size).batch(BATCH_SIZE)
...
def generate_images(num_images):
    ...
    generated_images = (generated_images * 127.5 +
                        127.5).numpy().astype(np.uint8)
